...
import com.example.blog.model.Request.UserLoginRequest;
import com.example.blog.model.User;
import com.example.blog.model.UserDTO;
import com.example.blog.repository.UserRepository;
import com.example.blog.security.Secured;
import com.example.blog.util.BCryptUtil;
...
public class UserHandler {
    ...
    public Response getAllUsers() {
        ...
        for (User user : users) {
            UserDTO userDTO = new UserDTO(user.getId(), user.getUsername(), user.getNickname(), user.getRole());
            if (userDTOS == null) {
    ...
    public UserDTO getUserById(@PathParam("id") int id /* 从路由的 {id} 中获得 id */) {
        ...
        return new UserDTO(user.getId(), user.getUsername(), user.getNickname(), user.getRole());
    }
    ...
    public Response login(UserLoginRequest request /* 接收的 JSON 实际上需要是一个 UserLoginRequest 的模型 */) {
        ...
        res.put("token", token);
        // 附加题1
        res.put("data", new UserDTO(user.getId(), user.getUsername(), user.getNickname(), user.getRole()));
        return Response.status(Response.Status.OK).entity(res).build();
    }
